...
class TestMXKeysSFallback(unittest.TestCase):
    """MX Keys S : pas de notification PUSH → fallback (this_mac_host+1) % num_hosts."""

    @patch("swigi.path_push.get_host_info", return_value=(3, 0))
    @patch("swigi.path_push._reconnect_keyboard")
    def test_fallback_on_disconnect_no_push(self, mock_reconnect, mock_get_host):
        """Déco sans notification PUSH (drain vide) → fallback hôte suivant."""
        keyboard = _make_keyboard(name="MX Keys S", product_id=0xB378)
        mock_reconnect.return_value = None  # stop après déco
        ...
        # Ping échoue → déco, drain vide → fallback
        keyboard.transport.write.side_effect = TransportError("dead")
        keyboard.transport.read.return_value = None  # drain vide
        ...
        push_events = [e for e in events if e.source == "push"]
        self.assertGreaterEqual(len(push_events), 1)
        # this_mac_host=0, num_hosts=3 → fallback = (0+1)%3 = 1
        self.assertEqual(push_events[0].target_host, 1)

    @patch("swigi.path_push.get_host_info", return_value=(3, 0))
    @patch("swigi.path_push._reconnect_keyboard")
    def test_no_fallback_if_push_notification_received(self, mock_reconnect, mock_get_host):
        """Notification PUSH reçue dans la session → pas de fallback MX Keys S."""
        ...
        # D'abord une notification PUSH (target=2), puis déco
        ...
        keyboard.transport.write.side_effect = mock_write
        keyboard.transport.read.side_effect = [notif_packet, None, None]  # notif, puis drain vide

        with patch("swigi.path_push._PING_INTERVAL", 999.0), patch(
            "swigi.path_push._READ_WINDOW", 0.01
        ...
        events = list(event_queue.queue)
        push_events = [e for e in events if e.source == "push"]
        # Seulement l'event de la notification originale (target=2), pas de fallback target=1
        targets = [e.target_host for e in push_events]
        self.assertNotIn(1, targets, "fallback ne doit pas émettre si notification déjà reçue")

    @patch("swigi.path_push.get_host_info", return_value=(3, 1))
    @patch("swigi.path_push._reconnect_keyboard")
    def test_fallback_wraps_correctly_last_host(self, mock_reconnect, mock_get_host):
        """this_mac_host=2 (dernier hôte) → fallback = (2+1)%3 = 0."""
        keyboard = _make_keyboard(name="MX Keys S", product_id=0xB378, change_host_index=10)
        mock_reconnect.return_value = None
        ...
        # this_mac_host=1 depuis get_host_info, mais on patch num_hosts=3, host=2
        with patch("swigi.path_push.get_host_info", return_value=(3, 2)), patch(
            "swigi.path_push._PING_INTERVAL", 0.0
        ), patch("swigi.path_push._READ_WINDOW", 0.01), patch(
            "swigi.state._STABILITY_WAIT", 0.0
        ):
            thread = threading.Thread(
                target=watch_keyboard_push,
        ...
        push_events = [e for e in events if e.source == "push"]
        self.assertGreaterEqual(len(push_events), 1)
        # this_mac_host=2, num_hosts=3 → fallback = (2+1)%3 = 0
        self.assertEqual(push_events[0].target_host, 0)
