...
## Architecture (7 modules principaux)
...
| `daemon.py`    | Orchestrateur PUSH + dispatcher + helpers partagés (\_reconnect_keyboard) |
...
Sur macOS Bluetooth, le kernel peut fermer le handle HID AVANT que la notification CHANGE_HOST soit lisible en userspace. Le code maximise le temps passé dans `hid_read_timeout()` (fenêtre 500ms) pour capturer cette notification avant que le kernel ferme le handle. Si la souris ne suit pas, le probe retente automatiquement (toutes les 2-3s pendant 30s). Au-delà du timeout, l'utilisateur re-appuie sur Easy-Switch.
...
## Détection stale post-reconnexion clavier

Après reconnexion BLE, le firmware Logitech redelivre la dernière notification CHANGE_HOST (0.5–3s après connexion). Pour distinguer une notification stale d'un vrai switch Easy-Switch, on utilise un ping actif (150ms timeout) : si le clavier répond, c'est stale ; s'il ne répond pas, il est en train de déconnecter (switch réel). Pas de fenêtre temporelle fixe.
