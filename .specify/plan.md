...
## Phase 7 — Correction bugs critiques dual-path (T39–T44)

**Date :** 2026-05-29
**Motivation :** Audit post-implémentation Phase 6. Cinq bugs critiques bloquent le fonctionnement réel du dual-path : ping-pong entre périphériques, BetterMouse silencieux, switches perdus.

### Bugs identifiés post-Phase 6

| ID   | Sévérité | Fichier                | Description                                                                                      |
| ---- | -------- | ---------------------- | ------------------------------------------------------------------------------------------------ |
| B-P1 | CRITIQUE | `path_pull.py`         | Aucun `_SwitchEvent` posté — souris ne suit jamais sur Legacy. `_SwitchEvent` non importé.       |
| B-P2 | CRITIQUE | `path_pull.py`         | Pas de `get_current_host` post-reconnect — target host inconnu même si B-P1 corrigé.             |
| B-P3 | CRITIQUE | `path_pull.py`         | Watchdog inatteignable — `last_response = time.time()` après write réussi, check toujours < 10s. |
...
### 7.1 Fix `path_pull.py` — implémentation PULL complète

**Ce qui manque :**

1. Import `_SwitchEvent`
2. `get_current_host()` sur le nouveau handle après reconnect → `this_mac_host`
3. `event_queue.put(_SwitchEvent(this_mac_host, name, "pull"))`
4. `hunt_trigger.set()`
5. Watchdog : `last_response` mis à jour sur **lecture de réponse** (pas sur write)
...
try:
    ...
    # disconnect → reconnect → get_current_host → post SwitchEvent
    ...

# Lire réponse ping (met à jour watchdog)
raw = keyboard.transport.read(timeout=100)
if raw and len(raw) >= 4:
    last_response = time.time()

# Watchdog (now reachable)
if time.time() - last_response > _WATCHDOG_TIMEOUT:
    ...
```

### 7.2 Fix `daemon.py` — état post-dispatch et hunt_trigger
...
# path_pull.py
_READ_TIMEOUT_PING = 100  # ms — timeout lecture réponse ping (pour watchdog)
```
